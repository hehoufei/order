package com.mklinfo.aegean.investment.invest;
...
public class TestWithoutSpring {
    ...
    public void array() {
        ...
        for (int i = 0; i < nums.length; i++) {
            if (hash.containsKey(nums[i])) {
                duplicatedNums[0] = i;
                duplicatedNums[1] = hash.get(nums[i]);
            }
            // 将数据存入 key为补数 ，value为下标
            hash.put(target - nums[i], i);
        }
        System.out.println(duplicatedNums);
    ...
    public void test1() {
        ...
        int[][] queries = new int[][]{{0,1}, {1, 2}, {0, 3}, {0, 3}};
        System.out.println(Arrays.toString(this.solution(arr, queries)));
    }
